    ...
    def spac_Spatial():
        ...
        if adata is not None:
            if slide_check is False and region_check is False:
                out = spac.visualization.interative_spatial_plot(adata, annotations=input.spatial_anno(), figure_width=5.5, figure_height=5, dot_size=input.spatial_slider())
            ...
            if slide_check is True and region_check is False:
                ...
                out = spac.visualization.interative_spatial_plot(adata_subset, annotations=input.spatial_anno(), figure_width=5.5, figure_height=5, dot_size=input.spatial_slider())
                out[0]['image_object'].update_xaxes(showticklabels=True, ticks="outside", tickwidth=2, ticklen=10)
                out[0]['image_object'].update_yaxes(showticklabels=True, ticks="outside", tickwidth=2, ticklen=10)
                return out[0]['image_object']
