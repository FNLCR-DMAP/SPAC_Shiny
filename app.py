...
app_ui = ui.page_fluid(
    ...
    ui.navset_card_tab(
        ...
        ui.nav_panel("Data Input",
            ...
            # We can keep this layout since it's mostly for inputs, not a big visualization.
            ui.row(
                ui.column(6,
        ...
        ui.nav_panel("Features",
            ui.card(
                {"style": "width:100%;"},
                ui.row(
                    ui.column(3,  # side panel for inputs
                    ...
                    ui.column(9,  # main area for plot
...
def server(input, output, session):
    ...
    # Subsetting logic + subset history -----------------------------------------------
    subset_ui_initialized = reactive.Value(False)

    @reactive.effect
    def subset_reactivity():
        # Manage dynamic UI for subsetting
    ...
    # Trigger to force label dropdown updates
    label_update_trigger = reactive.Value(0)

    @reactive.effect
    def update_subset_labels():
        trigger = label_update_trigger.get()
        adata = adata_main.get()
    ...
    # Master copy of the loaded data for restoration ----------------------------------
    ...
    def spac_Histogram_1():
        ...
        # Case 1: Not grouping by anything
        ...
        # Case 2: Grouping by an annotation
        else:
            # If "Plot Together" is on, use the user's dropdown choice
            if input.h1_together_check():
                multiple_param = input.h1_together_drop()  # 'layer', 'stack', 'fill', or 'dodge'
                together_flag = True
            else:
                # If grouping by but not "plot together," pick a default layout (e.g. "layer" or "dodge")
    ...
    def update_stack_type_dropdown():
        if input.h1_together_check():
            dropdown_together = ui.input_select(
                "h1_together_drop", "Select Stack Type",
    ...
    def spac_Boxplot_1():
        ...
        if adata is not None and adata.var is not None:
            if input.bp1_layer() != "Original" and input.bp1_anno() != "No Annotation":
                fig, ax, df = spac.visualization.boxplot(
                    adata, annotation=input.bp1_anno(),
                    layer=input.bp1_layer(),
                    features=list(input.bp1_features()),
                    showfliers=input.bp1_outlier_check()
                )
            ...
            if input.bp1_layer() == "Original" and input.bp1_anno() == "No Annotation":
                fig, ax, df = spac.visualization.boxplot(
                    adata, features=list(input.bp1_features()),
                    showfliers=input.bp1_outlier_check()
                )
                return ax.legend(loc='upper right', bbox_to_anchor=(1, 1))
        return None
    ...
    def spac_Boxplot_2():
        ...
        if adata is not None and adata.var is not None:
            if input.bp2_layer() != "Original" and input.bp2_anno() != "No Annotation":
                fig, ax, df = spac.visualization.boxplot(
                    adata, annotation=input.bp2_anno(),
                    layer=input.bp2_layer(),
                    features=list(input.bp2_features()),
                    showfliers=input.bp2_outlier_check()
                )
                return ax.legend(loc='upper right', bbox_to_anchor=(1, 1))
            if input.bp2_layer() == "Original" and input.bp2_anno() != "No Annotation":
                fig, ax, df = spac.visualization.boxplot(
            ...
            if input.bp2_layer() == "Original" and input.bp2_anno() == "No Annotation":
                fig, ax, df = spac.visualization.boxplot(
                    adata, features=list(input.bp2_features()),
                    showfliers=input.bp2_outlier_check()
                )
                return ax.legend(loc='upper right', bbox_to_anchor=(1, 1))
        return None

    # HISTOGRAM 2 (Annotations panel) -------------------------------------------------
    @output
    @render.plot
    ...
    def spac_Histogram_2():
        adata = adata_main.get()
        if adata is not None:
            if input.h2_group_by_check():
                if input.h2_together_check():
                    fig1 = spac.visualization.histogram(
                        adata,
                        annotation=input.h2_anno(),
                        group_by=input.h2_anno_1(),
                        together=input.h2_together_check(),
                        multiple=input.h2_together_drop()
                    )
                    return fig1
                else:
                    fig1 = spac.visualization.histogram(
    ...
    def update_stack_type_dropdown2():
        if input.h2_together_check():
            dropdown_together = ui.input_select(
                "h2_together_drop", "Select Stack Type",
                choices=['stack', 'layer', 'dodge', 'fill'],
                selected='stack'
            )
            ui.insert_ui(
    ...
    def spac_Heatmap():
        adata = ad.AnnData(X=X_data.get(), obs=pd.DataFrame(obs_data.get()),
                           var=pd.DataFrame(var_data.get()),
                           layers=layers_data.get(), dtype=X_data.get().dtype)
        if adata is not None:
            ...
            if not input.dendogram():
                if input.hm1_layer() != "Original":
                    df, fig, ax = spac.visualization.hierarchical_heatmap(
                        ...
                        z_score=None, vmin=vmin, vmax=vmax
                    )
                    return fig
                else:
                    df, fig, ax = spac.visualization.hierarchical_heatmap(
                        adata, annotation=input.hm1_anno(),
                        layer=None, z_score=None, vmin=vmin, vmax=vmax
                    )
                    return fig
            else:
                ...
                if input.hm1_layer() != "Original":
                    df, fig, ax = spac.visualization.hierarchical_heatmap(
                        adata, annotation=input.hm1_anno(),
                        layer=input.hm1_layer(), z_score=None,
    ...
    def spac_UMAP():
        adata = ad.AnnData(X=X_data.get(), var=pd.DataFrame(var_data.get()),
                           ...
                           dtype=X_data.get().dtype, layers=layers_data.get())
        ...
        if adata is not None:
            if input.umap_rb() == "Feature":
                ...
                out = spac.visualization.dimensionality_reduction_plot(
                    adata, method=input.plottype(),
                ...
                out1 = spac.visualization.dimensionality_reduction_plot(
                    adata, method=input.plottype(),
            ...
            if btn == "Annotation":
                ...
                if not umap_feature_initialized.get():
                    ...
                    table_umap = ui.input_select(
                        "umap_layer", "Select a Table",
                        choices=new_choices, selected=["Original"]
                    )
                    ui.insert_ui(
    ...
    def spac_Spatial():
        adata = ad.AnnData(X=X_data.get(), obs=pd.DataFrame(obs_data.get()),
                           obsm=obsm_data.get(), dtype=X_data.get().dtype)
        ...
        if adata is not None:
            if not slide_check and not region_check:
                out = spac.visualization.interative_spatial_plot(
                    ...
                    figure_width=4, figure_height=4,
                    dot_size=input.spatial_slider()
                )
                out[0]['image_object'].update_xaxes(
                    showticklabels=True, ticks="outside", tickwidth=2, ticklen=10
                )
                out[0]['image_object'].update_yaxes(
                    showticklabels=True, ticks="outside", tickwidth=2, ticklen=10
                )
                return out[0]['image_object']
            if slide_check and not region_check:
                ...
                return out[0]['image_object']
            if slide_check and region_check:
                adata_subset = adata[
                ...
                return out[0]['image_object']
            if not slide_check and region_check:
    ...
    def get_scatterplot_names():
        if obsm_names.get() is not None and var_names.get() is not None:
            ...
            d = {"Annotated Tables": obsm_dict, "Features": features_dict}
            return d
        return []
